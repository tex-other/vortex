...
\ifx\initex\undefined  \def\initex{} % next lines are skipped if format loaded
  ...
  \catcode `\^^? = 0 % and so will a strange escape delimiter
  ...
  \font\bigtrip=trip at 20pt % this will be extension font
  \textfont3=\bigtrip \chardef\?=`b
  \patterns{\?50aa1b3 *1aca. bb bb1 0b2b0 b1c} % remember that *==space
  \hyphenation\relax{b-\?-\char`b -b-b-b tiny }
  \skip200 = 10pt plUs5fillllminus 0 fill
  \setbox200=\vbox{\hrule\vskip\skip200} \wd200-2pt \setbox100=\hbox{A}
  \skipdef\shkip100\shkip -18pt plus\catcode`\}fil minus 10fil
  \advance\shkip by \skip200
  \dimen33=3pt
  \count33=-\dimen33
  \divide\shkip by \count33
  \multiply\shkip by \count33 % so \skip100=-6pt plus 3filll minus 9fil
  \count200 -5
  \multiply\count200 by -100 % \count200 is 500
  \count100=1000000
  \divide\count100 by \count200 % \count100 is 2000
  \dimen100=,00152587890625in % (100/65536)in = 7227sp
  \multiply\dimen100 by 65536
  \divide\dimen100 by 9 % \dimen100 is 803pt
...
\openout-'78terminal \openout10=tripos % we will write three lines on tripos.tex
%%%%%%% Incremental formatting test No. 2 %%%%%%%%%
\write10{} % the first line is empty
...
\setbox255\vbox{}
\dimen200=10000pt
%%%%%%% Incremental formatting test No. 3 %%%%%%
{\output{\dimen 9=\ht200\count5=\dimen9
    \global\countz=\outputpenalty
    \setbox255\copy255 % at end of group, \box255 reverts to former value
    \shipout\hbox{\box100\box200\vsplit 255 to 55pt}
    \unvcopy255\showlists\showthe\insertpenalties
    \message{\topmark:\firstmark:\botmark:\splitfirstmark:\splitbotmark}
    \globaldefs1\halign{#\tabskip\lineskip\cr}
    \showboxdepth1\showboxbreadth2}
...
%%%%%% Incremental formatting test No. 4 %%%%%%111
...
\tracingmacros=1
\def\t12#101001#{-.#1pt}
\dimendef\varunit=222\varunit=+1,00101pt
...
%%%% Incremental formatting test No. 5 %%%%%
...
A /A\char`A BBBBCACAC//% that becomes /k[AA]k[BB]k[BB][CA][CA][C/]/,
 ...
 % if I use the notation {x|y|z} for \discretionary{x}{y}{z}. (one hyphen lost)
...
%%%%%% Incremental formatting test No. 6 %%%%%%%
...
{\rightskip0pt plus 104pt minus 100fil
  ...
  A\spacefactor32767\discretionary{}{-}{B\kern2pt} C$ \scriptfont2=\trip
  \mathsurround143pt$ C $\mathsurround40pt$$\mathsurround60pt\hbox{$$}$\par}
%%%%% Incremental formatting test No. 7 %%%%%%%%
...
\message{\fontname\ip}
\rip
\textfont1=\font \scriptfont1=\smalltrip \scriptscriptfont1=\bigtrip % [sic]
...
%%%%%%% Incremental formatting test No. 8 %%%%%%%
\begingroup
\looseness 2
...
\halign to\the\displaywidth{#&#\crcr\crcr\cr} % the display counts as lines 4--6
...
\mkern-9mu \the\prevgraf \prevgraf=8 \insert255{\penalty999}
...
%%%%%% Incremental formatting test No. 9 %%%%%%%
...
\mathord \radical"161 % missing { will be inserted
  {\textstyle\radical"282382{\left(\scriptscriptstyle\mathop{\underline{
      ...
      \right[A}}}
\let\penalty=\minus
\eqno\scriptstyle\penalty % reader, be alert
(\mathpunct{AA}
|{B\fam1-}^{\hbox{A}}{\above9pt{u\overwithdelims..
    \displaystyle{A\atopwithdelims((\vrule height 9pt}}
  \show\penalty
  \showlonglists$$ % end of the hairy display, missing } will be inserted
...
\endgroup % this restores \parshape
...
\vtop to\displaywidth{\everydisplay{\global}\vbox to -1sp{}\noindent$$
  \count9=\predisplaysize\lowercase{AaJ}$$}\hss
\leqno\mathchardef A\/\left(\over\left($$
...
\vbox to 10pt{\hbadness 99\hfuzz1pt\hbox to 0pt{\hskip 10pt minus 9pt}
  \hbadness100\hbox to 10bp{\hskip 0pt plus 10pt}\tracingcommands1 \if00\fi}
\lineskiplimit-1pt\everyhbox{}
%%%%%% Incremental formatting test No. 10 %%%%%%
...
\halign spread-12.truedd{&#\span\iftrue\A\span\else\span\fi\span&
  ...
  \global\edef\A{\uppercase{
      \message{\fontname\smalltrip\the\font\romannumeral1009}\lowercase{uq}} }
...
%%%%%% Incremental formatting test No. 11XY %%%%%%
^^?a^^@^^@a\par{\lo\par % runaway in use
...
%%%%%% Incremental formatting test No. 12X %%%%%%
\hbox{\/\hrule\textfont3=\enorm\prevdepth\advance\xspaceskip by-\xspaceskip
  ...
  $62{}\delimiterfactor1600\left(Aa$AA\/}
...
\def\a#1{\ifcat#1 \message\ifx#1 {\the\tokens\fi\fi}}
\pretolerance-1\tokens\toks1\unhbox16\par\everycr{\noalign{\penalty97}}
\ifcase1\or\ifeof\fi\def\stopinput{\error\let\input\die}
...
\relax \catcode`\^^M=13 \defqqM{\relax}#\begingroup{\showboxdepth=4\showbox10}

\long\def\l#1\l{#1}\immediate\write10{\string\caution \l} % living dangerously
\escapechar`|\tracingoutput0\shipout\vbox{\copy10\box10}
...
\escapechar127\global\tracingoutput1\global\escapechar128\end
% things not tested:
